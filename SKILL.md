...
- 禁止编造产品功能、价格、用户数据、市场份额、融资信息、发布日期、客户案例和第三方评价。
- 没有来源的功能、价格、数据和日期，不要写成确定事实；必须标记为“未查证”“待验证”或“不确定”。
- 报告中的结论区分三类：事实发现、分析判断、行动建议。不要把分析判断包装成客观事实。
- 所有评分矩阵、雷达图、四象限和排行都必须写清评分口径，说明分数来自哪些维度和证据。
- 图表只能基于报告中已有事实、表格或评分生成，不能凭空生成新数据。
- 如果联网失败、用户禁止联网或只能基于指定材料分析，必须在报告开头说明资料边界和时效风险。
- 涉及“最新”“当前”“最近”“价格”“政策”“AI 能力”“市场数据”等易变化信息，必须优先查证并标注来源或访问口径。
- 来源不足时，优先缩小结论范围；不要用模糊措辞掩盖证据不足。
...
- 关键任务路径对比不要写成整段长文；优先使用表格，或按“关键流程 / 竞品差异 / 行动判断”分点分行。
- 在优先级建议中使用 `Must:`、`Should:`、`Could:` 开头的条目，脚本会渲染为三列优先级看板。
- 也可以使用 `优先级 | 建议 | 动作说明` 表格，第一列填 Must / Should / Could，脚本会按行生成优先级看板。
- 来源列表优先使用短标题 + Markdown 链接，不要把超长 URL 当作主要展示文本；HTML 脚本会把来源表自动渲染为紧凑卡片。
...
- 对比分析：按目标选择维度，不堆无关表格。
- 关键洞察：明确区分事实发现、分析判断、机会和风险。
- 行动建议：做什么、不做什么、优先级、下一步。
- 评分口径：说明矩阵、图表、排行和优先级建议的判断依据。
- 来源列表：列出使用的主要来源、访问日期或访问口径。
- 未查证项：列出资料不足、可能过时或需要用户后续验证的信息。

产品/体验类报告还应包含：

- 核心功能矩阵。
- 关键任务路径对比：用表格或分点呈现，每条路径至少拆成关键流程、竞品差异和行动判断，避免大段叙述。
- 体验问题清单。
- 差异化功能总结。
