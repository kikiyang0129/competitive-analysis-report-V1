...
7. 输出报告框架，等待用户确认。
8. 正式查证和生成报告。
9. 若生成 HTML，可使用 `scripts/build_html_report.py` 将 Markdown 转为自包含可视化 HTML；脚本只依赖 Python 标准库。

## HTML 可视化规则

当用户选择 HTML 可视化报告时，生成 Markdown 草稿时尽量使用下列结构，方便脚本自动渲染图表：

- 在 `## 摘要结论` 下使用 3-6 条短 bullet，脚本会渲染为摘要卡片。
- 使用 1-5 分评分矩阵，第一列为维度，后续列为竞品，脚本会渲染为热力表格、雷达图、四象限和综合评分排行。
- 在关键任务路径中使用 `步骤一 → 步骤二 → 步骤三`，脚本会渲染为流程图。
- 在优先级建议中使用 `Must:`、`Should:`、`Could:` 开头的条目，脚本会渲染为三列优先级看板。
- 自动图表最多生成 3 组，避免深度报告页面过重。
